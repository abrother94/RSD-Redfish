...
MessagePrivilegeRegistryFile::MessagePrivilegeRegistryFile(const std::string& path) : EndpointBase(path) { }
MessagePrivilegeRegistryFile::~MessagePrivilegeRegistryFile() { }
#define UNUSED(x) (void)(x)

void MessagePrivilegeRegistryFile::get(const server::Request& request, server::Response& response) 
{
    //Only 1 privilege files and its file ID is 2 //
    UNUSED(request);
    const auto& file = registries::MessageRegistryFileManager::get_instance()->get_file_by_id(2);
    Json::Value Jsons = file.get_pri_json(); 
